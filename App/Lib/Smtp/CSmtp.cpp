...
{
	{command_INIT,          0,/*0*/     5*60,  220, ECSmtp::SERVER_NOT_RESPONDING},
	...
	{command_DATABLOCK,     3*60,  0,/*0*/0,/*0*/   ECSmtp::COMMAND_DATABLOCK},	// Here the valid_reply_code is set to zero because there are no replies when sending data blocks
...
void CSmtp::Send()
{
	...
	try{
		...
		{
			// opening the file:
			sprintf (AttachAllPath, "%s", Attachments[FileId].c_str());
		...
		// send attachments from ddr
		if (NULL != m_pAttDate || NULL != m_pAttName || 0 < m_Attlen)
		{
			//RFC 2047 - Use UTF-8 charset,base64 encode.
			EncodedFileName = "=?UTF-8?B?";
			EncodedFileName += base64_encode(reinterpret_cast<const unsigned char*>(m_pAttName), strlen(m_pAttName));
			EncodedFileName += "?=";

			snprintf(SendBuf, BUFFER_SIZE, "--%s\r\n", BOUNDARY_TEXT);
			//strcat(SendBuf, "Content-Type: application/x-msdownload; name=\"");
			strcat(SendBuf, "Content-Type: text/plain; name=\"");
			strcat(SendBuf, EncodedFileName.c_str());
			strcat(SendBuf, "\"\r\n");
			strcat(SendBuf, "Content-Transfer-Encoding: base64\r\n");
			strcat(SendBuf, "Content-Disposition: attachment; filename=\"");
			...
			MsgPart = 0;
			for(i = 0;m_Attlen - i*54 > 0;i++){
				res = (m_Attlen - i*54 >= 54) ? 54 : m_Attlen - i*54;
				memcpy(FileBuf, &m_pAttDate[i*54],res);
				MsgPart ? strcat(SendBuf,base64_encode(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str())
					: strcpy(SendBuf,base64_encode(reinterpret_cast<const unsigned char*>(FileBuf),res).c_str());
				strcat(SendBuf,"\r\n");
				MsgPart += res + 2;
...
void CSmtp::FormatHeader(char* header)
{
	...
	snprintf(header, BUFFER_SIZE, "Date: %d %s %d %d:%d:%d+8\r\n", timeinfo->tm_mday,
	         month[timeinfo->tm_mon], timeinfo->tm_year+1900, timeinfo->tm_hour,
             timeinfo->tm_min, timeinfo->tm_sec); 
	printf ("Mail time is [%s]", header);
...
void CSmtp::AttachmentInfo(char *AttName, char *AttDate, int AttLen)
{
	if (NULL == AttName || NULL == AttDate || 0 >= AttLen) return;
	m_Attlen = AttLen;
	m_pAttName = AttName;
	m_pAttDate = AttDate;
}